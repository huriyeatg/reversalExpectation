function plot_binxaveragey_bilateral_M2stimulation(input,tlabel,bin,save_path)
% % plot_binxaveragey_bilateral_M2stimulation %
%PURPOSE:   Make scatter plot
%AUTHORS:   H Atilgan & AC Kwan 
%
%INPUT ARGUMENTS
%   input:      Paired data
...
for k=1:nData
    ...
    for j = 1:nbin
        ...
        plot(j+1.05,nanmean(y2),'.','MarkerSize',30,'color',[0 0 1]);
        plot([j+0.95 j+0.95],nanmean(y)+nanstd(y)/sqrt(numel(y))*[-1 1],'k-','LineWidth',3);
        plot([j+1.05 j+1.05],nanmean(y2)+nanstd(y2)/sqrt(numel(y2))*[-1 1],'-','LineWidth',3,'color',[0 0 1]);
    ...
    plot([1:nbin]+1.1,y2Mean,'-','color',[0 0 1])
    xlim([1.5 nbin+1.5]);
    xticks([2:nbin+1])
    xticklabels(str)
    ...
    xlabel(label{1});
    ylabel(label{2});
    legend('control','stimulated','location','southwest');
    legend box off;
...
if strcmp(label{2}{1},'P(stay|win)')
    print(gcf,'-dsvg',fullfile(save_path,'Pstay'));
    saveas(gcf, fullfile(save_path,'Pstay'), 'fig');
    
elseif  strcmp(label{2}{1},'P(lose|switch)')
    print(gcf,'-dsvg',fullfile(save_path,'Plose'));
    saveas(gcf, fullfile(save_path,'Plose'), 'fig');
else  
    print(gcf,'-dsvg',fullfile(save_path,label{2}{1}));
...
% hTable = figure(1);
% if strcmp(label{2}{1},'P(stay|win)')
%     print(hTable,'-dsvg',fullfile(save_path,'Pstay_statsOutput'));
% elseif  strcmp(label{2}{1},'P(lose|switch)')
%     print(hTable,'-dsvg',fullfile(save_path,'Plose_statsOutput'));
% else
%     print(hTable,'-dsvg',fullfile(save_path,[label{2}{1},'_statsOutput']));
% end
