...
stats.pWinStay =  nan(numel(stats.blockLength)-1,1);
stats.pLooseSwitch =  nan(numel(stats.blockLength)-1,1);
stats.hitrates = nan(numel(stats.blockLength)-1,1);
stats.rewardrates = nan(numel(stats.blockLength)-1,1);
...
for i=1:numel(stats.blockLength)-1
    ...
    if ~isempty(idx)
        ...
        %p(win(rewarded)|stay)
        rewardedTrials = stats.r((sum(stats.blockLength(1:i))-5) :sum(stats.blockLength(1:i))-1); %stats.r((sum(stats.blockLength(1:i-1))+1):(sum(stats.blockLength(1:i))));
        nextTrialStay  = diff(stats.c((sum(stats.blockLength(1:i))-5) :sum(stats.blockLength(1:i)))==stats.hr_side(sum(stats.blockLength(1:i-1))+1))==0;
        stats.pWinStay(i)     = nansum( rewardedTrials==1 & nextTrialStay==1)/ nansum(rewardedTrials==1);
        stats.pLooseSwitch(i) = nansum( rewardedTrials==0 & nextTrialStay==0)/ nansum(rewardedTrials==0);
        
        % hit/reward rates
        stats.hitrates(i)    = nansum(stats.c((sum(stats.blockLength(1:i-1))+1):(sum(stats.blockLength(1:i))+1))==stats.hr_side(sum(stats.blockLength(1:i-1))+1))/numel((sum(stats.blockLength(1:i-1))+1):(sum(stats.blockLength(1:i))+1));
        stats.rewardrates(i) = nansum(stats.r((sum(stats.blockLength(1:i-1))+1):(sum(stats.blockLength(1:i))+1)))/numel((sum(stats.blockLength(1:i-1))+1):(sum(stats.blockLength(1:i))+1));
        stats.hitrates(i) = stats.hitrates(i)*100; % percent
        stats.rewardrates(i) = stats.rewardrates(i)*100; % percent
    end 
end
%% check consistency
