function output=choice_switch_hrsideSelected(stats,trials_back)
...
for i=1:numel(stats.blockLength)-1
    ...
    if ~isnan(idx) && stats.blockSelected(i)==1  %NaN could arise if we are using merge_session
        
        trial1=(sum(stats.blockLength(1:i))+1)-trials_back;
        trial2=(sum(stats.blockLength(1:i))+1)+trials_back;
        if trial2<=numel(stats.c)
            
            % if left side stimulated & switch is from left to right
