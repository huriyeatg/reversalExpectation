...
for k=1:numRange
    %plot the mean
    h(legendNum) = plot(n,probh(:,k),'.-','MarkerSize',10,'Linewidth',3,'Color',cmap(k,:));
    %if more than 1 data set, then plot the error bars
    if numel(input)>1
        for l=1:numel(n)
            plot(n(l)*[1 1],probh(l,k)+probh_sem(l,k)*[-1 1],'-','LineWidth',3,'Color',cmap(k,:));
        end
    end
    
    legstring{legendNum}=['Initial better option (L_C: ' int2str(L1_ranges(k,1)) '-' int2str(L1_ranges(k,2)) '; L_R: ' int2str(L2_ranges(k,1)) '-' int2str(L2_ranges(k,2)) ')'];
...
for k=1:numRange
    %plot the mean
    h(legendNum) = plot(n,probl(:,k),'v-','MarkerSize',5,'Linewidth',3,'Color',cmap(11-k,:));
    %if more than 1 data set, then plot the error bars
    if numel(input)>1
        for l=1:numel(n)
            plot(n(l)*[1 1],probl(l,k)+probl_sem(l,k)*[-1 1],'-','LineWidth',3,'Color',cmap(11-k,:));
        end
    end
    
    legstring{legendNum}=['Initial worse option (L_C: ' int2str(L1_ranges(k,1)) '-' int2str(L1_ranges(k,2)) '; L_R: ' int2str(L2_ranges(k,1)) '-' int2str(L2_ranges(k,2)) ')'];
    legendNum = legendNum + 1;
end
...
for k=1:numRange
    %plot the mean
    h(legendNum) = plot(n,probn(:,k),'.-','MarkerSize',10,'Linewidth',3,'Color',k/(numRange+1)*[1 1 1]);
    %if more than 1 data set, then plot the error bars
    if numel(input)>1
        for l=1:numel(n)
    ...
    legstring{legendNum}=['Miss (L_C: ' int2str(L1_ranges(k,1)) '-' int2str(L1_ranges(k,2)) '; L_R: ' int2str(L2_ranges(k,1)) '-' int2str(L2_ranges(k,2)) ')'];
    legendNum = legendNum + 1;
end
