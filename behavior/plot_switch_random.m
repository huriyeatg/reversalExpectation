...
for j=1:numtransType
    ...
    for k=1:numRange
        %plot the mean
        h(legendNum) = plot(n,probl(:,j,k),'.-','MarkerSize',30,'Linewidth',3,'Color',cmap(k,:));
        %if more than 1 data set, then plot the error bars
        if numel(input)>1
            for l=1:numel(n)
                plot(n(l)*[1 1],probl(l,j,k)+probl_sem(l,j,k)*[-1 1],'-','LineWidth',3,'Color',cmap(k,:));
            end
        end
        
        legstring{legendNum}=['Left (L_C: ' int2str(L1_ranges(k,1)) '-' int2str(L1_ranges(k,2)) '; L_R: ' int2str(L2_ranges(k,1)) '-' int2str(L2_ranges(k,2)) ')'];
    ...
    for k=1:numRange
        %plot the mean
        h(legendNum) = plot(n,probr(:,j,k),'v-','MarkerSize',6,'Linewidth',3,'MarkerFaceColor', cmap(k,:),'Color',cmap(k,:));
        %if more than 1 data set, then plot the error bars
        if numel(input)>1
            for l=1:numel(n)
                plot(n(l)*[1 1],probr(l,j,k)+probr_sem(l,j,k)*[-1 1],'-','LineWidth',3,'Color',cmap(k,:));
            end
        end
                
        legstring{legendNum}=['Right (L_C: ' int2str(L1_ranges(k,1)) '-' int2str(L1_ranges(k,2)) '; L_R: ' int2str(L2_ranges(k,1)) '-' int2str(L2_ranges(k,2)) ')'];
        legendNum = legendNum + 1;
