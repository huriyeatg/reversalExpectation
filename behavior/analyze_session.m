...
print(gcf,'-dsvg',fullfile(save_path,'switches_hrside'));    
...
sw_randomVsChoice.label{1}={'L_R (random number of trials added)'};
sw_randomVsChoice.label{2}={'Fraction of trials';'selecting initial worse option'};

plot_binxaveragey(sw_randomVsChoice,tlabel);
print(gcf,'-dpng',fullfile(save_path,'switches_LRVsChoice'));
saveas(gcf, fullfile(save_path,'switches_LRVsChoice'), 'fig');

%% plot logistic regression analysis
num_regressor = 10;
[lreg, ~, ~, ~]=logreg_RCUC(stats,num_regressor);
