function output=choice_switch_hrside_randomSelected(stats,trials_back,L1_ranges,L2_ranges)
...
for i=1:numel(stats.blockLength)-1
    ...
    if ~isnan(idx)  %NaN could arise if we are using merge_session
        ...
        if sum(range_idx) == 1  && stats.blockSelected(i)==1  %if this transition falls into one of the subset of switches to be considered
            ...
            if trial2<=numel(stats.c)
               % if   (stats.blockSt(i)==0 && stats.blockSelected(i)==1) || (stats.blockSt(i)==1 && idx ==2  && stats.blockSelected(i)==1) || (stats.blockSt(i)==2 && idx ==1  && stats.blockSelected(i)==1)
       % stats.blockSelected(i)==1
