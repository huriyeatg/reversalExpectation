...
for j = 1:numel(animalList)
   ...
   % stats.blockTrialRandomAdded = stats.blockLength;
    ...
    %% plot choice behavior - REAL BLOCK LENGTH
    L1_ranges=[10 20;10 20;10 20;10 20]; %consider only subset of blocks within the range, for trials to criterion
    L2_ranges=[10 15;16 20; 21 30; 31 50];% [ 15 19;20 24;25 29;30 45];%   %consider only subset of blocks within the range, for random added number of trials    
    
    sw_hrside_blocklength_output{j}=choice_switch_hrside_blocklength(stats,trials_back,L1_ranges,L2_ranges);
    ...
    ind = find(stats.blockTrialtoCrit<=Lcriteria & stats.blockRule(1:end-1)==2);
    sw_LRVsChoiceR{j}.dat=[stats.blockTrialRandomAdded(ind) stats.blockPreSwitchBetterChoiceAtSwitch(ind)];
    sw_LRVsChoiceR{j}.range{1}=[0 30];
    sw_LRVsChoiceR{j}.range{2}=[0.6 1];
    sw_LRVsChoiceR{j}.label{1}={'L_{Random}'};
    sw_LRVsChoiceR{j}.label{2}={'Fraction of trials';'selecting initial better option'};
    

    ind = find(stats.blockTrialtoCrit<=Lcriteria & stats.blockRule(1:end-1)==1);
    sw_LRVsChoiceL{j}.dat=[stats.blockTrialRandomAdded(ind) stats.blockPreSwitchBetterChoiceAtSwitch(ind)];
    sw_LRVsChoiceL{j}.range{1}=[0 30];
    sw_LRVsChoiceL{j}.range{2}=[0.6 1];
    sw_LRVsChoiceL{j}.label{1}={'L_{Random}'};
    sw_LRVsChoiceL{j}.label{2}={'Fraction of trials';'selecting initial better option'};
...
plot_switch_hrside_random(sw_hrside_blocklength_output,tlabel);
%legend off
print(gcf,'-dpng',fullfile(save_path,'switches_hrside_blocklength-all-LC200'));
saveas(gcf, fullfile(save_path,'switches_hrside_blocklength-all-LC200'), 'svg');
...
plot_multibinxaveragey([ {sw_LRVsChoiceL},{sw_LRVsChoiceR},...sw_LRVsMidpoint
...
% Used for final version of comparison
% save_path = '/Users/Huriye/Documents/Code/bandit/figs/alldata-4datapoints';
% L2_ranges = [0 4;5 9;10 14;15 30];
% tlabelBilateral   = [{'Control'}; {'Stimulated'}];
% plot_binxaveragey_bilateral_M2stimulation([{sw_LRVsChoiceL,sw_LRVsChoiceL};...
%     {sw_LRVsChoiceR},{sw_LRVsChoiceR}],tlabelBilateral, L2_ranges,save_path);
