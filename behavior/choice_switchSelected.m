...
numruletrans=size(stats.ruletransList,1)+1;
    ...
    for i=1:numel(stats.blockLength)-1
        ...
        if  ~isnan(idx) && stats.blockSelected(i)==1   %NaN could arise if we are using merge_session
            ...
            if trial2<=numel(stats.c)   %if trial after switch does not exceed end of session
                ...
                chosel(:,idx)=chosel(:,idx)+(stats.c(trial1:trial2) == stats.hr_side(trial1));
                choser(:,idx)=choser(:,idx)+(stats.c(trial1:trial2) == -1*stats.hr_side(trial1));
