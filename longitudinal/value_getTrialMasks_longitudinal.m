function [ trials ] = value_getTrialMasks_longitudinal( trialData )
% % value_getTrialMasks_longitudinal
%PURPOSE:   Create data structures: 'trials' containing logical masks, and
%           'stats' containing dummy codes for task variables.
%AUTHORS:   H Atilgan 211030
%
%INPUT ARGUMENTS
...
for i = 1:numel(taskVar)
    ...
    switch taskVar{i}
        ...
        case 'rule'
            ...
            elseif trialData.presCodeSet<=3   || trialData.presCodeSet==8 ||...
                    trialData.presCodeSet==21 || trialData.presCodeSet==22 ||...
                    trialData.presCodeSet==31 || trialData.presCodeSet==32
...
if sum(trials.reward)+sum(trials.noreward)+sum(trials.miss) ~= nTrials
    ...
    disp('ERROR in value_getTrialMasks: check #1 reward+no reward+miss = nTrial');
elseif sum(trials.left)+sum(trials.right)+sum(trials.miss) ~= nTrials
    ...
    warning('ERROR in value_getTrialMasks: check #2 left+right+miss = nTrial');
end
