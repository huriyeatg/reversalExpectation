function bandit_lesionPerSession(dataIndex,save_path)
% % bandit_lesionPerSession %
...
for kk = 1:2
    if kk==1     %pre-lesion sessions
        currList = preLesionList;
    elseif kk==2 %post-lesion sessions
        currList = postLesionList;
    end
    ...
    for j = 1:numel(currList)
        ...
        if dataIndex.LesionSide(currList(j))==1  %if lesioned side is LEFT
        ...
        ind1 = stats.blockTrialtoCrit<20 & stats.blockTrans==1; % general criteria
        ind2 = stats.blockTrialtoCrit<20 & stats.blockTrans==2;
        ...
        ind = stats.blockTrialtoCrit<=20 & stats.blockTrans==1; %general criteria
        ...
        ind = stats.blockTrialtoCrit<=20 & stats.blockTrans==2; % general criteria
...
plot_behperf_lesion_hitRates(dat(1).hitRates,dat(2).hitRates,tlabel);
