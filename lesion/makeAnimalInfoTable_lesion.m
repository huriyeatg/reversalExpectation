function InfoTable = makeAnimalInfoTable_lesion(dataIndex)
% % makeAnimalInfoTable_lesion %
...
%% Per Animal
animalList = unique(dataIndex.Animal(dataIndex.Lesioned==1));
...
for b = 1:nAnimal
   ...
   currAnimalSessions = ismember(dataIndex.Animal,animalList(b)) & ~isnan(dataIndex.Lesioned);
    % Animal ID
    ind = find(currAnimalSessions==1);
    InfoTable.Animal(b) = dataIndex.Animal(ind(1));
    InfoTable.LesionSide(b) = dataIndex.LesionSide(ind(1));
